...
export class BillingService {
  ...
  async changeSubscriptionPlan(
    ...
    if (!tenantHasManageableSubscription(tenant)) {
      throw new BadRequestException(
        'Você ainda não possui uma assinatura ativa. Escolha um plano abaixo para assinar.',
      );
    }

    const targetTier = normalizePlanTier(planTier);
    const currentTier = normalizePlanTier(tenant.plan_tier);

    if (targetTier === currentTier) {
      throw new BadRequestException('Você já está neste plano.');
    }

    const subscriptionId = tenant.stripe_subscription_id?.trim();

    if (!subscriptionId) {
      throw new BadRequestException(
        'Assinatura não encontrada. Sincronize o status ou assine um plano.',
      );
    }
    ...
    if (!subscriptionItem) {
      throw new InternalServerErrorException(
        'Não foi possível localizar o item de assinatura no Stripe.',
      );
    }

    const newPriceId = this.resolveStripePriceId(targetTier);
    ...
    try {
      ...
      const synced = await this.syncSubscriptionFromStripe(updatedSubscription, {
        applyPlanTierOnActive: true,
      });

      if (!synced) {
        throw new InternalServerErrorException(
          'Plano alterado no Stripe, mas não foi possível atualizar o estabelecimento.',
        );
      }

      this.logger.log(
        `Plan changed: tenant=${tenant.id} ${currentTier} -> ${targetTier}`,
      );
      ...
      this.rethrowStripeCheckoutError(error, newPriceId);
    }
  }
